...
#    Updated: 2022/04/12 11:02:34 by gneve            ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
do 
	...
	echo $(curl -s -o /dev/null -w '%{content_type}' "$1encoded$i.html") >> log/testn"$i".log
	echo $(curl -s -o /dev/null -w "%{time_total}\n" -H "Pragma: no-cache" "$1encoded$i.html") >> log/loadtimen"$i".log
	echo $(curl -s  "$1encoded$i.html") | wc -c >> log/contentln"$i".log
